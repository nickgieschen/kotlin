...
public class RenameKotlinPropertyProcessor : RenamePsiElementProcessor() {
    override fun canProcessElement(element: PsiElement): Boolean = element.unWrapToProperty() is JetProperty

    /* Can't properly update getters and setters in Java */
    override fun isInplaceRenameSupported() = false

    override fun substituteElementToRename(element: PsiElement?, editor: Editor?): PsiElement? {
        val jetProperty = element?.unWrapToProperty() as? JetProperty
    ...
    private fun addRenameElements(psiMethod: PsiMethod?,
        ...
        OverridingMethodsSearch.search(psiMethod, scope, true).forEach { overrider ->
            val overriderElement = overrider.unWrapToProperty() as? PsiNamedElement
    ...
    private fun findDeepestOverriddenProperty(jetProperty: JetProperty): JetProperty? {
        if (jetProperty.getModifierList()?.hasModifier(JetTokens.OVERRIDE_KEYWORD).equals(true)) {
            ...
            if (descriptor != null) {
                ...
                // Take one of supers for now - API doesn't support substitute to several elements
    ...
    private fun PsiElement.unWrapToProperty(): PsiElement {
        val unwrappedElement = this.unwrapped
        return when (unwrappedElement) {
            is JetProperty -> unwrappedElement
            is JetPropertyAccessor -> PsiTreeUtil.getParentOfType(unwrappedElement, javaClass<JetProperty>()) ?: this
            else -> this
        }
    }
}